...
A Variational Learner, a subtype of the abstract [`Speaker`](@ref) type.
"""
...
Constructs a [`VariationalLearner`](@ref) with learning rate `g` and advantage matrix `A`.
...
Constructs a classical 2-grammar (i.e. 1-parameter) [`VariationalLearner`](@ref) with learning rate `g` and advantage parameters `a1` and `a2`.
...
Called by [`listen!`](@ref), this function is unlikely to be of use in itself to the end user.
"""
...
Make [`VariationalLearner`](@ref) learn from input token `y`, which is simply a grammar index.

Called by [`listen!`](@ref), this function is unlikely to be of use in itself to the end user. Implements Bush and Mosteller's Linear Reward-Penalty Learning algorithm, generalized for n dimensions.
"""
...
Make [`VariationalLearner`](@ref) `x` listen to one utterance from `y` and learn based on this input experience.
